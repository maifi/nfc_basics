...
public class IaikCryptaTag implements ICryptoTag {
	...
	public byte[] getCert(){
		if (this.currentIsoDep == null) {
			Log.d("NFC-CryptoTag", "No tag in proximity.");
			return null;
		}
		byte[] cert = null;
		
		// select file
		if (!selectFile(this.currentIsoDep, FILEID_CERT)) {
			Log.d("NFC-CryptoTag", "NFC: Read Cert failed!\n");
			return null;
		}
		
		cert = readBinary(this.currentIsoDep, (short)0,(byte)0);
		if(cert == null){
			Log.d("NFC-CryptoTag", "Reading Cert failed");
			return null;
		}
		Log.d("NFC-CryptoTag", "Reading Cert OK!");
		return cert;
	}
	...
	public byte[] getChallenge(){
		...
		try {
			response = this.currentIsoDep
					.transceive(GET_CHALLENGE);
		} catch (IOException e) {
			Log.e("NFC-CryptoTag", "NFC: IOException caught during transceive(): "
					+ e.getMessage());
			return null;
		}
		Log.d("NFC-CryptoTag", "Get Challenge OK!");
		return response;
	}
	...
	public byte[] getUid() {
		...
		if (!selectFile(this.currentIsoDep, FILEID_UID)) {
			Log.d("NFC-CryptoTag", "NFC: Read UID failed!\n");
			return null;
		}
		...
		Log.d("NFC-CryptoTag", "NFC: UID (length=" + uid_final.length + "): "
				+ Utils.byteArrayToHexString(uid_final) + "\n");

		// end
		Log.d("NFC-CryptoTag", "NFC: UID successfully read!\n");
		return uid_final;
	}
	private boolean selectFile(IsoDep isoDep, short ID) {
		...
		try {
			...
			Log.d("NFC-CryptoTag", "NFC: IOException caught during transceive(): "
					+ e.getMessage());
			return (false);
		}
		...
		if (response.length != 2) {
			...
			if (response[0] == (byte) 0x90 && response[1] == (byte) 0x00) {
				Log.d("NFC-CryptoTag", "NFC: File selection successful!\n");
				return (true);
			} else if (response[0] == (byte) 0x6A && response[1] == (byte) 0x82) {
				Log.d("NFC-CryptoTag", "NFC: File or application not found!\n");
				return (false);
			} else if (response[0] == (byte) 0x69 && response[1] == (byte) 0x82) {
				Log.d("NFC-CryptoTag", "NFC: Security status not satisfied!\n");
				return (false);
			} else if (response[0] == (byte) 0x6D && response[1] == (byte) 0x00) {
				Log.d("NFC-CryptoTag", "NFC: Command not supported or invalid!\n");
				return (false);
			} else {
				Log.d("NFC-CryptoTag", "NFC: Unkown response!\n");
				return (false);
			}
		}
	}
	...
	private byte[] readBinary(IsoDep isoDep, short offset, byte length) {
		...
		if (response == null || response.length == 0) {
			...
			if (response[response.length - 2] == (byte) 0x90
				...
				Log.d("NFC-CryptoTag", "NFC: Read binary successful!\n");
				...
				return (Arrays.copyOfRange(response, 0, response.length - 2));
					...
					&& response[response.length - 1] == (byte) 0x82) {
				Log.d("NFC-CryptoTag", "NFC: Security status prevents reading!\n");
				return (null);
	...
	public byte[] sign(byte[] data) {
		...
		// check input
		if (data == null) {
			Log.e("NFC-CryptoTag", "NFC: Invalid input! Null value not accepted!");
			return null;
		}
		if (data.length != TagCrypto.CRYPTO_TAG_NONCE_LENGTH) {
			Log.e("NFC-CryptoTag", "NFC: Invalid input! Length must be 16!\n");
			return null;
		}
		...
		if (response == null || response.length == 0) {
			...
			if (response[response.length - 2] == (byte) 0x90
				...
				Log.e("NFC-CryptoTag", "NFC: Unkown response!");
				return null;
			}
			...
			if (response[response.length - 2] == (byte) 0x62
				...
				return null;
					...
					&& response[response.length - 1] == (byte) 0x82) {
				Log.e("NFC-CryptoTag", "NFC: Security status prevents reading!");
				return null;
			} else if (response[response.length - 2] == (byte) 0x6D
					&& response[response.length - 1] == (byte) 0x00) {
				Log.e("NFC-CryptoTag", "NFC: Command not supported or invalid!");
				return null;
			} else {
				Log.e("NFC-CryptoTag", "NFC: Unkown response!");
				return null;
